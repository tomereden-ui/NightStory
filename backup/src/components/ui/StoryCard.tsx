      ...
      <Link href={`/player?id=${story.id}`} className="block group flex-shrink-0">
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 p-3" style={{ background: "linear-gradient(to top, rgba(0,0,0,0.85) 0%, transparent 100%)" }}>
            {story.isFavorite && <span className="text-pink text-xs">♥ </span>}
            <h3 className="text-white text-xs font-semibold leading-tight line-clamp-2">{title}</h3>
            <div className="flex items-center gap-1.5 mt-1">
              <span className="text-teal/70 text-[10px]">{story.voice.avatarEmoji}</span>
              <span className="text-white/40 text-[10px]">{formatDuration(story.durationSeconds)}</span>
            </div>
          </div>

          {/* Category badge */}
          <div className="absolute top-2.5 left-2.5">
            <span className="text-[9px] font-medium uppercase tracking-wider bg-black/40 backdrop-blur-sm text-white/60 px-2 py-0.5 rounded-full border border-white/10">
              {story.category}
            </span>
          </div>
        </div>
    ...
    return (
      <Link href={`/player?id=${story.id}`} className="block group">
        <div className="flex items-center gap-3 p-3 rounded-2xl bg-bg-card border border-bg-border transition-all group-hover:border-purple/30 group-hover:shadow-purple-sm group-active:scale-[0.99]">
          {/* Cover */}
          <div
            className="w-14 h-14 rounded-xl flex items-center justify-center text-3xl flex-shrink-0 border border-white/5"
            style={coverStyle}
          >
            {story.coverEmoji}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-white/35 text-xs mt-0.5 truncate">{description}</p>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-teal/60 text-[10px]">
                {story.voice.avatarEmoji} {language === "he" ? story.voice.nameHe : story.voice.name}
              </span>
              <span className="text-white/20 text-[10px]">·</span>
              <span className="text-white/30 text-[10px]">{formatDuration(story.durationSeconds)}</span>
            </div>
          </div>
          ...
          <div className="w-8 h-8 rounded-full border border-purple/40 flex items-center justify-center flex-shrink-0 group-hover:bg-purple/15 group-hover:border-purple/70 transition-all">
            <span className="text-purple-bright text-xs ml-0.5">▶</span>
          </div>
        </div>
      </Link>
    );
    ...
    <Link href={`/player?id=${story.id}`} className="block group flex-shrink-0">
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-2.5" style={{ background: "linear-gradient(to top, rgba(0,0,0,0.85) 0%, transparent 100%)" }}>
          <h3 className="text-white text-xs font-semibold leading-tight line-clamp-2">{title}</h3>
          <p className="text-white/40 text-[10px] mt-0.5">{formatDuration(story.durationSeconds)}</p>
        </div>
      </div>
    </Link>
  );
