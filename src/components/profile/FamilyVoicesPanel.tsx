...
function VoiceCard({
  ...
  /** Whether there's SOMETHING to play — the voice's own raw clone sample, or
   *  (when that's missing) a cached canned-line sample generated via the
   *  Admin "Generate Voice Preview Samples" tool. Defaults to sample_url
   *  alone when the cache hasn't loaded yet. */
  canPreview?: boolean;
}) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(255,255,255,0.03)", border: editOpen ? "1px solid rgba(79,195,247,0.25)" : "1px solid rgba(255,255,255,0.07)" }}>
      ...
      <div className="flex items-center gap-3 px-4 py-3.5">
        ...
        <button disabled={!canPreview} onClick={() => onPlay(voice)} className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-all active:scale-95" style={{ background: isPlaying ? "rgba(79,195,247,0.15)" : "rgba(255,255,255,0.05)", color: isPlaying ? "#4fc3f7" : canPreview ? "rgba(255,255,255,0.45)" : "rgba(255,255,255,0.15)", border: isPlaying ? "1px solid rgba(79,195,247,0.4)" : "1px solid rgba(255,255,255,0.08)", cursor: canPreview ? "pointer" : "not-allowed" }}>
...
export default function FamilyVoicesPanel() {
  ...
  const [mounted, setMounted] = useState(false);
  // Per-(voice, language) canned-line samples generated by the Admin "Generate
  // Voice Preview Samples" tool — same cache the Studio voice picker and the
  // narrator picker use. A cloned voice's own recorded sample_url (its actual
  // clone clip) is always preferred when present; this is only a fallback for
  // a family voice that has no raw sample of its own yet.
  ...
  const resolvePreviewUrl = useCallback((voice: VoiceRecord): string | undefined => {
    const lang = PREVIEW_LANGUAGE_SET.has(language) ? language : "en";
    return voice.sample_url || sampleMap[voice.id]?.[lang];
  }, [language, sampleMap]);
  ...
  }, [playingId, stopAudio, resolvePreviewUrl]);
            ...
            <div className="flex flex-col gap-2 mb-3">
              {voices.map((v) => (
                <VoiceCard
                  ...
                  canPreview={!!resolvePreviewUrl(v)}
                />
              ))}
              <button
                onClick={() => setShowAdd(true)}
